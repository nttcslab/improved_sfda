...
class ImageList(datasets.VisionDataset):
    ...
    def __init__(self, root: str, classes: List[str], data_list_file: str = None, data_list: List = None,
                 transform: Optional[Callable] = None, target_transform: Optional[Callable] = None, transform_t: Optional[Callable] = None, M:Optional[int] = 1, K:Optional[int] = 0):
        ...
        if data_list_file is not None:
            ...
            raise ValueError("either data_list_file or data_list should be filled")
    ...
    def __getitem__(self, index: int) -> Tuple[Any, int]:
        ...
        if self.transform is not None:
            if self.M == 1:
                ...
                img_a = [self.transform(img) for _ in range(self.M)]
        if self.target_transform is not None and target is not None:
            target = self.target_transform(target)

        if self.transform_t is not None and self.K != 0:
            img_b = [self.transform_t(img) for _ in range(self.K)]
            return img_a, img_b, target, index
        else:
            return img_a, target, index
