...
def slicd_Wasserstein_distance(x1, x2, n_projection=128, mask=None):
    ...
    if mask is not None:
        mask = mask.flatten(-2).transpose(1, 2).contiguous()
        B, N, _ = mask.size()
        mask = mask.expand(B, N, n_projection)
        mean_square = (sorted_proj_x1 - sorted_proj_x2).pow(2)
        masked_mean_square = mean_square * mask
        result = masked_mean_square.sum() / mask.sum()
    else:
        result = (sorted_proj_x1 - sorted_proj_x2).pow(2).mean()
    return result

def generate_mask(x):
    grayscale_x = rgb_to_grayscale(x)
    mask = ((grayscale_x <= 0.999) * (grayscale_x >= 0.001)).to(torch.float32)
    return mask

class AugmentationContainer(nn.Module):
    def __init__(
            self, c_aug, g_aug, c_reg_coef=0,
            normalizer=None, replay_buffer=None, n_chunk=16, use_mask=False):
        super().__init__()
        self.c_aug = c_aug
        self.g_aug = g_aug
        ...
        if update and self.c_reg_coef > 0:
            if self.normalizer is not None:
                if self.use_mask:
                    ...
                    mask = None
                swd = self.c_reg_coef * slicd_Wasserstein_distance(self.normalizer(x), self.normalizer(aug_x), mask=mask)
            else:
                if self.use_mask:
                    mask = generate_mask(x)
                else:
                    mask = None
                swd = self.c_reg_coef * slicd_Wasserstein_distance(x, aug_x, mask=mask)
        else:
